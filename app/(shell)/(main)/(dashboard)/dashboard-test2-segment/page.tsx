...
const App = () => {
    ...
    const matchedSegments: UserSegmentResponse[] = 
        dataSegments?.matchedSegments?.map((segment: { id: any; userId: any; segmentId: any; segment: { id: any; slug: any; name: any; post: string | number | boolean | JsonObject | JsonArray | null; product: { id: any; name: any; description: any; price: any; category: any; quantity: any; primaryPhoto: any; imageGallery: any; createdAt: { toString: () => any; }; updatedAt: { toString: () => any; }; metadata: string | number | boolean | JsonObject | JsonArray | null; ogImage: any; }; domainId: any; }; }) => ({
            id: segment.id,
                ...
                <ul>
                    {matchedSegments.map((segment) => (
                        <div key={segment.segment?.id || segment.id} style={{ border: '1px solid #ccc', padding: '16px', margin: '8px 0', borderRadius: '8px' }}>
                            ...
                            <div>
                                <strong>Segment ID:</strong> {segment.segment?.id}
                            </div>
                            <div>
                                <strong>Product Name:</strong> {segment.segment?.product?.name}
                            </div>
                            <div>
                                <strong>Description:</strong> {segment.segment?.product?.description}
                            </div>
                            <div>
                                <strong>Price:</strong> ${segment.segment?.product?.price}
                            </div>
                            <div>
                                <strong>Category:</strong> {segment.segment?.product?.category}
                            </div>
                            <div>
                                <strong>Quantity:</strong> {segment.segment?.product?.quantity}
                            </div>
                            <div>
                            ...
                            <div>
                                <strong>Created At:</strong> {segment.segment?.product?.createdAt ? new Date(segment.segment.product.createdAt).toLocaleDateString() : 'N/A'}
                            </div>
                            <div>
                                <strong>Updated At:</strong> {segment.segment?.product?.updatedAt ? new Date(segment.segment.product.updatedAt).toLocaleDateString() : 'N/A'}
                            </div>
                        </div>
                    ))}
                </ul>
