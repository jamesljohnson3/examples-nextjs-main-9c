...
const ProductPage: React.FC = () => {
  ...
  const handleAddCustomField = () => {
    ...
    if (customFieldType === 'select' && customFieldOptions) {
      newField.options = customFieldOptions.split(',').map(option => option.trim());
    }
    ...
    <div>
      ...
      {loadingProduct ? <p>Loading product...</p> : (
        <div>
          ...
          <Button onClick={handlePublish} disabled={segments.length === 0}>
            Publish Segments
          </Button>
